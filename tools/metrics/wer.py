  ...
  for i in range(1, target_len + 1):
    for j in range(1, predicted_len + 1):
      if target[i-1] == predicted[j - 1]:
        dp[i][j][3] = copy.copy(dp[i - 1][j - 1][3]) + 1
      else:
        prev_costs = [dp[i - 1][j - 1], dp[i - 1][j], dp[i][j - 1]]
        add_costs = [SUB, DEL, INS]
        cost_sub = get_cost(prev_costs[0]) + add_costs[0]
        cost_del = get_cost(prev_costs[1]) + add_costs[1]
        cost_ins = get_cost(prev_costs[2]) + add_costs[2]
        index = argmin([cost_sub, cost_del, cost_ins])

        set_cost(dp, i, j, prev_costs[index])
